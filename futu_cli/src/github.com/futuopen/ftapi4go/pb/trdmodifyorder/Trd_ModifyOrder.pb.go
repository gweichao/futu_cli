...
type C2S struct {
	...
	ModifyOrderOp int32                `protobuf:"varint,4,req,name=modifyOrderOp" json:"modifyOrderOp"`
	ForAll        bool                 `protobuf:"varint,5,opt,name=forAll" json:"forAll"`
	//下面的字段仅针对单个订单，且modifyOrderOp为ModifyOrderOp_Normal有效
	Qty   float64 `protobuf:"fixed64,8,opt,name=qty" json:"qty"`
	Price float64 `protobuf:"fixed64,9,opt,name=price" json:"price"`
	//以下为调整价格使用，都传才有效，对港、A股有意义，因为港股有价位，A股2位精度，美股可不传
	AdjustPrice        bool    `protobuf:"varint,10,opt,name=adjustPrice" json:"adjustPrice"`
	AdjustSideAndLimit float64 `protobuf:"fixed64,11,opt,name=adjustSideAndLimit" json:"adjustSideAndLimit"`
}
...
var fileDescriptor_9c54c3ea975941a8 = []byte{
	...
	,
}
...
func (m *C2S) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	...
	i--
	dAtA[i] = 0x28
	i = encodeVarintTrd_ModifyOrder(dAtA, i, uint64(m.ModifyOrderOp))
	i--
	dAtA[i] = 0x20
	i = encodeVarintTrd_ModifyOrder(dAtA, i, uint64(m.OrderID))
	i--
	dAtA[i] = 0x18
	if m.Header == nil {
	...
	} else {
		{
			...
			i -= size
			i = encodeVarintTrd_ModifyOrder(dAtA, i, uint64(size))
		}
		i--
		dAtA[i] = 0x12
	}
...
func (m *Response) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	...
	i = encodeVarintTrd_ModifyOrder(dAtA, i, uint64(m.ErrCode))
	i--
	dAtA[i] = 0x18
	i -= len(m.RetMsg)
	copy(dAtA[i:], m.RetMsg)
	i = encodeVarintTrd_ModifyOrder(dAtA, i, uint64(len(m.RetMsg)))
	i--
	dAtA[i] = 0x12
	if m.RetType == nil {
		return 0, github_com_gogo_protobuf_proto.NewRequiredNotSetError("retType")
	} else {
		i = encodeVarintTrd_ModifyOrder(dAtA, i, uint64(*m.RetType))
		i--
		dAtA[i] = 0x8
	}
	return len(dAtA) - i, nil
...
func encodeVarintTrd_ModifyOrder(dAtA []byte, offset int, v uint64) int {
	offset -= sovTrd_ModifyOrder(v)
	base := offset
	for v >= 1<<7 {
...
func (m *C2S) Size() (n int) {
	...
	if m.PacketID != nil {
		l = m.PacketID.Size()
		n += 1 + l + sovTrd_ModifyOrder(uint64(l))
	}
	if m.Header != nil {
		l = m.Header.Size()
		n += 1 + l + sovTrd_ModifyOrder(uint64(l))
	}
	n += 1 + sovTrd_ModifyOrder(uint64(m.OrderID))
	n += 1 + sovTrd_ModifyOrder(uint64(m.ModifyOrderOp))
	n += 2
	n += 9
	n += 9
	n += 2
...
func (m *Response) Size() (n int) {
	...
	_ = l
	if m.RetType != nil {
		n += 1 + sovTrd_ModifyOrder(uint64(*m.RetType))
	}
	l = len(m.RetMsg)
	n += 1 + l + sovTrd_ModifyOrder(uint64(l))
	n += 1 + sovTrd_ModifyOrder(uint64(m.ErrCode))
	if m.S2C != nil {
		l = m.S2C.Size()
		n += 1 + l + sovTrd_ModifyOrder(uint64(l))
	}
...
func sovTrd_ModifyOrder(x uint64) (n int) {
	return (math_bits.Len64(x|1) + 6) / 7
}
func sozTrd_ModifyOrder(x uint64) (n int) {
	return sovTrd_ModifyOrder(uint64((x << 1) ^ uint64((int64(x) >> 63))))
}
func (m *C2S) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		for shift := uint(0); ; shift += 7 {
			if shift >= 64 {
				return ErrIntOverflowTrd_ModifyOrder
			}
			if iNdEx >= l {
		...
		case 1:
			...
			if msglen < 0 {
				return ErrInvalidLengthTrd_ModifyOrder
			}
		...
		case 4:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field ModifyOrderOp", wireType)
			}
			m.ModifyOrderOp = 0
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
				...
				b := dAtA[iNdEx]
				iNdEx++
				m.ModifyOrderOp |= int32(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
		...
		default:
			iNdEx = preIndex
			skippy, err := skipTrd_ModifyOrder(dAtA[iNdEx:])
			if err != nil {
				return err
			}
			if skippy < 0 {
	...
	if hasFields[0]&uint64(0x00000008) == 0 {
		return github_com_gogo_protobuf_proto.NewRequiredNotSetError("modifyOrderOp")
	}
...
func skipTrd_ModifyOrder(dAtA []byte) (n int, err error) {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		var wire uint64
		for shift := uint(0); ; shift += 7 {
			if shift >= 64 {
				return 0, ErrIntOverflowTrd_ModifyOrder
			}
			if iNdEx >= l {
		...
		case 2:
			...
			if length < 0 {
				return 0, ErrInvalidLengthTrd_ModifyOrder
			}
			iNdEx += length
			if iNdEx < 0 {
				return 0, ErrInvalidLengthTrd_ModifyOrder
		...
		case 3:
			for {
				...
				next, err := skipTrd_ModifyOrder(dAtA[start:])
				if err != nil {
					return 0, err
				}
				iNdEx = start + next
...
var (
	ErrInvalidLengthTrd_ModifyOrder = fmt.Errorf("proto: negative length found during unmarshaling")
	ErrIntOverflowTrd_ModifyOrder   = fmt.Errorf("proto: integer overflow")
)
